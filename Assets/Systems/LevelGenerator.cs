...
public class LevelGenerator : FSystem {
	...
	private void createTrap(int gridX, int gridY){
		GameObject trap = Object.Instantiate<GameObject>(Resources.Load ("Prefabs/Trap") as GameObject, gameData.Level.transform.position + new Vector3(gridY*3,0,gridX*3), Quaternion.Euler(0,0,0), gameData.Level.transform);
		trap.GetComponent<Position>().x = gridX;
		trap.GetComponent<Position>().y = gridY;
		GameObjectManager.bind(trap);
	}
